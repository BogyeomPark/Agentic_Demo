...
def ask_student_agent(chat_log):
    ...
    예시)<br>
    <ul>
      <li><b>데이터 사이언스 기초</b>: 데이터 기반 사고력과 분석 기초를 기를 수 있어요.</li>
      <li><b>인공지능 기초</b>: AI 시스템의 원리를 이해하고 기초 활용 능력을 쌓는 데 도움이 돼요.</li>
    </ul>
    <p>혹시 이 과목들에 관심이 있으신가요?</p>
    </li>
    
    <li>이제 지금까지 확인된 내용을 아래 형식으로 정리해서 마무리 멘트를 작성하세요:<br>
    <ul>
      <li><b>진로 방향:</b> 인공지능 개발자</li>
      <li><b>진학 학과:</b> 인공지능응용학과</li>
      <li><b>추천 과목:</b> 데이터 사이언스 기초, 인공지능 기초</li>
    </ul>
...
def ask_teacher_agent(teacher_input, chat_log):
    ...
    - 입력 예시: "관심 있다고 하네요"
    - 응답 예시:
    > **{{학생 이름}} 학생**이 해당 과목들에도 흥미를 보이고 있군요!  
    ...
    - 응답 예시:
    > {{학생 이름}} 학생이 {{유추된 학과}}에 관심을 보이고 있다면, 
    ...
    ※ 아래와 같은 형식으로 진학 전략을 **반드시 출력**하세요.  
    - 요약하지 말고, 항목마다 줄을 바꿔서 구체적으로 설명하세요.  
    - 아이콘(예: 🔹 🎯)과 굵은 제목(예: **교과전형 전략**)도 반드시 포함하세요.
